...
module SpecForker
  ...
  def self.forking_environment?
    !on_windows? && !on_jruby?
  end

  JAVA_PLATFORM_REGEXP = /java/.freeze
  def self.on_jruby?
    RUBY_PLATFORM =~ JAVA_PLATFORM_REGEXP
  end

  WIN_PLATFORM_REGEXP = /(:?mswin|mingw)/.freeze
  def self.on_windows?
    RUBY_PLATFORM =~ WIN_PLATFORM_REGEXP
  end
  ...
  def self.run(globs, spec_cmd='spec', run_opts = "-c", except = [])
    require "optparse"
    require "spec"
      ...
      globs.each do |glob|
        Dir[glob].each do |spec|
          if forking
            ...
            command = "NOW=1 #{Gem.ruby} #{File.dirname(__FILE__) + "/run_spec.rb"} \"#{spec}\""
            puts command
            `#{command}`
          end
